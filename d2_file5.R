...
#Assumptions
par(mfrow=c(2,2)) # multiple frames for row; displays the plots in the matrix of 2 by 2
plot(fit2)
par(mfrow=c(1,1)) # reset the plot display into single plot
...
omni[c(11,14,15),]
# Outliers are the values that dont fall near the regression and has high deviation from the model
fit3 = lm(sales ~ price + promotion, data=omni[-c(11,14,15),]) # remove the outliers to make the model more perfect and precise
plot(fit3,4)
summary(fit3)
