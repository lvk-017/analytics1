...
#sd of all columns 
sd(inputData$admit)
for (i in 1:4) { print(sd(inputData[,i]))} # for loop function
#use apply functions
sapply(inputData, sd)
lapply(inputData, sd)
...
#sapply :Simple apply & lapply to display the value in different formats
...
(t1= xtabs(~ admit + rank, data = data)) # xtabs helps in the display of the table in more detailed
addmargins(t1)
prop.table(t1,2) # proportion table
?prop.table
...
exp(coef(mylogit))
library(dplyr)
(ndata1 = sample_n(data, 3)) # from the data, 3 random data is selected
